...
> **Phase 17 (v2) — Independent Multi-Size Generation** (rework): HOTFIX-F reflow is superseded by a new `generateSizeVariant` callable (`functions/src/sizeVariant.ts`) that renders each requested size as a **fresh native design** for the target canvas, using the parent's saved build plan + a visual reference image (anchor for pre-select variants, own-original for resize, uploaded reference overrides both). Charges 5 credits per design upfront, refunds on failure, idempotency-keyed per `(genId, scope, itemIndex, ratio)`. Frontend fan-out is client-side, concurrency-capped at ≤10. See `specs/961-independent-multisize/` for the full spec/plan/research/contracts/tasks. The `reflowImage` callable is commented out in `functions/src/index.ts` (FR-021 — reversibility); the four HOTFIX-F module bodies (reflowImage/reflowRouter/reflowOutpaint/reflowRerender) carry a "Superseded by Phase 17 v2" header comment.
...
- The existing copy rules already handle FABRICATED HARD FACTS well (honest degradation in `hookAnglesKnowledge.ts`; NUMERIC FACT VIOLATION repair in `captionValidator.ts`) — those stay. What is missing is (a) explicit permission for invented FRAMING (scenarios/metaphors), which is orthogonal to the numeric guards, and (b) a soft-flag safety net for any fabricated specific that still slips through. Phase 22 adds those WITHOUT relaxing the compliance guards.
...
| 22.5 | `functions/src/generators.ts` | Add the FABRICATION POLICY block to the copy prompts (alongside, NOT replacing, the existing honest-degradation rules): "You may invent persuasive FRAMING freely — scenarios, hypotheticals, metaphors, illustrative composites. You do NOT need real proof to write persuasively. The existing honest-degradation rules still apply: if no real data is provided for a specific date, seat count, price, named person, or precise statistic, degrade honestly (cost-of-delay, structural scarcity, market-pattern proof) — do not invent those hard facts. If a fabricated verifiable specific still appears, the system will flag it for the user, not block it." | Fabrication-framing policy present. The existing honest-degradation lines in `hookAnglesKnowledge.ts` are NOT removed. No contradiction between the two. |
| 22.5b | `functions/src/hookAnglesKnowledge.ts` + `functions/src/captionValidator.ts` | DO NOT relax these. Confirm (and add a one-line code comment on each) that the honest-degradation rules (invented dates/seats/prices/names/stats → degrade) and the NUMERIC FACT VIOLATION repair are RETAINED as hard GCC/Meta compliance guards. The Phase 22 soft-flag is a safety net below them, not a replacement. | Both guards remain; comments note they are intentionally kept above the soft-flag layer. |
...
| 22.7 | `functions/src/generators.ts` | After copy generation, run a CLAIM DETECTOR over the 4 fields: flag any field containing a fabricated verifiable specific (named person, exact number/currency figure, hard headcount, star rating, concrete date/deadline/quantity). Populate `claimFlag[]` with `{ field, reason }`. Do NOT flag obvious hypotheticals, metaphors, illustrative scenarios, OR honestly-degraded framing (cost-of-delay, structural scarcity, market-pattern proof — these are already compliant, not fabrications). Never block or rewrite on a claim flag. | Copy with "Ahmed made 47,000 SAR in 30 days" → flagged. Copy with "imagine waking up to a full calendar" → not flagged. "every day you wait costs you clients" (cost-of-delay) → not flagged. Generation never blocked by a flag. |
...
| 22.13 | `functions/src/contractFixtures.test.ts` | Add copy-quality fixture tests: (a) reading-level rule present in `SYSTEM_TOV`, `HOOK_GENERATION_RULES`, carousel prompt, `RETARGETING_RULES`; (b) lived-symptom rule present in all four; (c) the fabrication-FRAMING policy is present AND the honest-degradation rules + NUMERIC FACT VIOLATION repair are STILL present (not removed); (d) claim detector flags a fabricated named-person/number, does not flag a hypothetical or honestly-degraded cost-of-delay line; (e) scoring pass returns reading-level + lived-symptom dimensions; (f) rewrite loop caps at 2 passes. | All 6 tests pass. |
...
### Phase 23.C — Carousel anti-sameness (story-angle choices + slide sequencing)

**Context (verified from code):** The carousel path still uses the OLD fixed angle map in TWO independent systems: (1) `generateCarouselAngles` in `generators.ts` hardcodes the 4 story-direction CHOICES as `A → Direct Value, B → Curiosity/Question, C → Social Proof/Story, D → Problem Agitation` (plus a retargeting variant), and `App.tsx` mirrors them in a hardcoded `angleLabels` map with fixed per-letter colors (A=blue, B=purple, C=green, D=orange) — this is what the user sees as the same four cards every project; (2) the MIDDLE-SLIDE sequencing uses a separate fixed pool `A=Direct value, B=Curiosity, C=Social proof, D=Problem agitation, E=Mechanism, F=Objection, G=Identity` assigned in lockstep order, defined in `LAUNCH_MATRIX.md` Section 5.A, `slidePlanEngine.ts`, and the committed spec 001 contract `carousel-slide-count-plan.md`. Both are being fixed. Temperature is already 0.85 here.

**Decisions (locked):** The carousel STAYS a multi-angle PICKER (it offers the user 4 different story directions to choose between — locking it to one angle would defeat the picker). The fix: (a) draw the 4 story-direction choices from a larger pool, rotated per project and biased by cross-project memory, so it is never the same 4 families every time; (b) the middle-slide sequencing also rotates which angles fill the arc rather than fixed A→B→C→D→E lockstep — while PRESERVING the correct invariants (no two adjacent slides repeat, CTA on slide 1 + last only, photo injection slide 1 only). Reuses the same `creativeMemory` from 23.B. **Because the slide-plan pool is a committed contract (spec 001) and lives in Section 5.A, those must be updated in lockstep with the code or the contract and code will disagree.**

| # | File | Action | Done when |
|---|---|---|---|
| 23.C1 | `functions/src/generators.ts` | In `generateCarouselAngles`, remove the hardcoded `A→Direct Value, B→Curiosity, C→Social Proof, D→Problem Agitation` choice map (and keep the retargeting variant behavior, but make it a pool too). Replace with a call to a story-direction selector (23.C2) that picks 4 directions from the full taxonomy pool. | No hardcoded 4-choice map remains; the 4 story directions come from the selector. |
| 23.C2 | `functions/src/creativeTextDirector.ts` | Add `selectCarouselStoryDirections(diagnosis, recentlyUsed, count=4)`: draw `count` distinct story directions from the full appropriate pool (cold pool incl. Direct Value, Curiosity, Social Proof, Problem Agitation, Mechanism, Identity, Cost of Inaction, Future, etc.; retargeting pool incl. Proof, Question Reframe, Identity Shift, Cost of Inaction, Risk Reversal, Evidence Comparison), down-weight directions in `recentlyUsed`, shuffle, never return the same ordered set twice in a row. Bias, never ban. | Returns 4 distinct on-strategy story directions; consecutive identical sets do not occur; pool never empties. |
| 23.C3 | `src/App.tsx` | Replace the hardcoded `angleLabels` + fixed per-letter color map with dynamic labels/colors driven by the story directions actually returned (the card shows whichever direction it is, not a fixed A=blue/B=purple mapping). | Cards display the rotated directions with sensible colors; no hardcoded 4-label map remains. |
| 23.C4 | `functions/src/slidePlanEngine.ts` | Change middle-slide angle assignment from fixed lockstep `A→B→C→D→E…` to a rotated draw from the angle pool, seeded by the chosen story direction + cross-project memory, while PRESERVING invariants: no two adjacent middle slides share an angle; CTA on slide 1 + last only; photo injection slide 1 only; deterministic given its seed (so the resolution trace stays reproducible). | Middle-slide angles rotate across projects; all existing invariants still hold; function still deterministic given seed. |
| 23.C5 | `functions/src/creativeMemory.ts` | Extend the 23.B memory to also record carousel story directions used + middle-slide angle sequences, per user across projects; expose them to 23.C2 and 23.C4 via the existing `getRecentHookUsage`-style getter. | Carousel directions + slide sequences recorded and retrievable for bias. |
| 23.C6 | `specs/001-resolver-completeness-trace/contracts/carousel-slide-count-plan.md` | Update the committed contract: the middle-slide angle assignment is now a SEEDED ROTATION from the pool, not fixed-order first-N-2. Document the preserved invariants (adjacency, CTA placement, photo injection, determinism-given-seed) and that the seed derives from story direction + memory. | Contract reflects rotation; invariants explicitly retained; no contradiction with 23.C4. |
| 23.C7 | `LAUNCH_MATRIX.md` Section 5.A | Update the cold + retargeting slide-count plan tables: replace the fixed "A → B → C → D → E in order" middle-slide column with "rotated draw from pool (seeded), no adjacent repeat." Keep the slide-count → CTA-placement structure unchanged. | Section 5.A shows rotated middle-slide assignment; CTA/structure rules unchanged. |
| 23.C8 | `functions/src/contractFixtures.test.ts` | Add carousel fixtures: (a) `selectCarouselStoryDirections` never returns the same ordered set twice consecutively; (b) a recently-used direction is down-weighted but a sole best-fit direction is still selectable; (c) `slidePlanEngine` middle-slide rotation never repeats an adjacent angle and keeps CTA on slide 1 + last only; (d) slide plan is deterministic given the same seed; (e) `App.tsx` renders dynamic labels (no fixed A=blue map); (f) two consecutive carousel projects differ in their 4 story directions. | All 6 tests pass. |
...
| ISSUE-D | Team member cannot see correct workspaces | `WorkspaceSwitcher` is not passed `isTeamMember` or `workspaceAccess`. No callable exists to load a team member's workspace access array. Team members see all workspaces instead of their assigned ones. | High | **Resolved by `965-team-workspace-access` (this branch).** `resolveCallerScope` now grants `"ALL"` to any verified team member; the stored per-member `workspaceAccess` array is not used for authorization, and read only when non-empty to emit the FR-004b override trace (FR-021). The four workspace-mutation callables are guarded by `assertNotTeamMember`. The team-screen access matrix and its mutation handler are removed (FR-020). The workspace fetch became a live `onSnapshot` that tears down on removal. Role-based editing was split to a follow-up spec per the 2026-07-27 clarification. Verification matrix lives at `specs/965-team-workspace-access/quickstart.md`. |
