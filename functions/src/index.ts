...
// callers in src/ AND zero Cloud Monitoring invocations in the 90-day window
// (cloudfunctions.googleapis.com/function/execution_count, project
// proadsai-saas). Stripe Customer Portal (createStripePortalSession) and the
...
// (Removed: restoreWorkspace callable. Zero callers in src/ AND zero
// Cloud Monitoring invocations in the 90-day window. The frontend
// never invokes it -- WorkspaceSettingsModal does not surface a restore
// button. The src/services/workspaceService.restoreWorkspace wrapper was
// removed in the type-only-exports batch. The cascadeRevertOnRestore
// helper stays: it is called from any future restore path (see
// cascadeReassignOnDelete above for the matching delete-side cascade).)
