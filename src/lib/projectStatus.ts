// src/lib/projectStatus.ts — deriveStatus: monotonic status latch for saved projects
...
export function deriveStatus(
  ...
  const result = (prevRank > derivedRank ? prev : derived) as ProjectStatus;
  if (prev !== result) {
    console.info(`phase13 ▸ status-latch ${prev ?? 'undefined'} → ${result} (derived=${derived})`);
  }
  return result;
}
