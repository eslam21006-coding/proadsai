...
Every Stripe webhook successfully processed by Firebase triggers a best-effort sync POST to GHL's inbound webhook URL. GHL uses the payload to run CRM automations (tagging, welcome emails, win-back, dunning). GHL sync failures never block the billing state update. Two distinct GHL webhook URLs: `GHL_STRIPE_SYNC_WEBHOOK_URL` for success events, `GHL_STRIPE_FAILED_WEBHOOK_URL` for dunning.
...
1. **Given** `checkout.session.completed` succeeds for an existing user (with `client_reference_id`), **When** GHL sync runs, **Then** GHL receives `{ email, contactName, plan, billingStatus, event, credits, stripeSubscriptionId }` (no `portalUrl`) — email and name read from `users/{uid}`.
2. **Given** `checkout.session.completed` succeeds for a pre-signup user (without `client_reference_id`), **When** GHL sync runs, **Then** GHL receives the same payload — email taken from `customer_details.email` (no Firebase Auth account exists yet).
3. **Given** `customer.subscription.deleted` succeeds, **Then** GHL receives `{ email, event: 'customer.subscription.deleted' }` to trigger win-back automation.
4. **Given** `invoice.payment_failed` succeeds, **Then** the failed-sync helper generates a fresh Stripe Customer Portal session URL transiently and GHL failed-sync receives `{ email, contactName, event, portalUrl }` on the dunning webhook URL.
5. **Given** `charge.refunded` succeeds (full refund), **Then** GHL failed-sync receives `{ email, contactName, event: 'refund_processed', portalUrl, amount, reason }` on the same dunning webhook URL.
6. **Given** transient portal session generation fails inside the failed-sync helper, **Then** the GHL POST is still made with `portalUrl` omitted and the failure is logged with code `portal_session_generation_failed`.
7. **Given** the GHL sync POST itself fails, **Then** the billing state update still completes and the failure is logged without throwing.
...
- **FR-018**: System MUST forward a best-effort sync POST to GHL after every successful Stripe webhook processing. Success sync goes to `GHL_STRIPE_SYNC_WEBHOOK_URL` with `{ email, contactName, plan, billingStatus, event, credits, stripeSubscriptionId }` — `portalUrl` is OMITTED from success-sync payloads (subscription created/updated/deleted, payment succeeded, top-up) because GHL's success automations do not need it. Dunning sync (for `invoice.payment_failed`) and refund sync (for `charge.refunded`) both go to `GHL_STRIPE_FAILED_WEBHOOK_URL` with `{ email, contactName, event, portalUrl, amount?, reason? }`; the sync helper MUST generate a fresh Stripe Customer Portal session URL transiently via `stripe.billingPortal.sessions.create({ customer: stripeCustomerId, return_url })` just before the POST so the URL is valid when the GHL email is opened. The sync helper MUST accept either a Firebase `uid` (reads user doc for email/name and `stripeCustomerId`) or a raw `email` string (for pre-signup users in `pending_plans`; portalUrl generation is skipped if no `stripeCustomerId` is available). Portal-session generation failures inside the sync helper MUST be logged with code `portal_session_generation_failed` but MUST NOT block the GHL POST — the payload is sent without `portalUrl` and GHL's email template MUST handle the missing field gracefully. GHL POST failures MUST be logged with code `ghl_sync_failed` but MUST NOT throw or block billing state updates.
...
- **FR-032**: System MUST handle `charge.refunded` webhooks. The handler MUST distinguish (a) full refund of a subscription invoice, (b) full refund of a top-up transaction, and (c) partial refund of either. For (a): the handler MUST programmatically cancel the user's subscription via the Stripe API using **immediate cancellation** (`stripe.subscriptions.cancel(stripeSubscriptionId)` with default parameters — no `cancel_at_period_end`, no `prorate`). The user has been refunded for the unused period, so continued access through period-end is unjustified. Stripe then fires `customer.subscription.deleted` and runs the existing cancellation flow (plan='none', credits=0, billingStatus='cancelled'). The handler MUST also POST a `refund_processed` event to `GHL_STRIPE_FAILED_WEBHOOK_URL` with `{ email, contactName, event: 'refund_processed', amount, reason }` so the CRM can trigger a refund-acknowledgement workflow. For (b): the handler MUST atomically deduct the refunded credit amount from `users/{uid}.credits` (clamped at zero), log the refund in `cancellation_logs` (or a new `refund_logs` collection — implementation decides), and send the `refund_processed` event to GHL. For (c) partial refunds: the handler MUST log the refund event but MUST NOT change plan or credits. All three branches MUST emit a structured log entry with classification code `refund_processed` and the refund amount, charge ID, and source (`subscription` vs `topup`).
