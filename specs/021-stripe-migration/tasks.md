...
- [ ] T011 [P] Create GHL "Stripe Payment Received" workflow with inbound webhook trigger; record URL as `GHL_STRIPE_SYNC_WEBHOOK_URL`; configure event-branch automations (`subscription.created` → tag + welcome, `subscription.deleted` → win-back, `subscription.updated` → tag refresh, `topup` → log) (quickstart B.3)
- [ ] T012 [P] Create GHL "Stripe Payment Failed / Refund" workflow with inbound webhook trigger; record URL as `GHL_STRIPE_FAILED_WEBHOOK_URL`; configure `past_due` → dunning email with `{{portalUrl}}` and `refund_processed` → refund acknowledgement automations (quickstart B.3)
...
- [ ] T014 Set Firebase Cloud Functions secrets: `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, `GHL_STRIPE_SYNC_WEBHOOK_URL`, `GHL_STRIPE_FAILED_WEBHOOK_URL` via `firebase functions:secrets:set` (quickstart C)
...
- [ ] T023 Add Stripe secrets to `functions/src/index.ts` imports: `defineSecret("STRIPE_SECRET_KEY")`, `defineSecret("STRIPE_WEBHOOK_SECRET")`, `defineSecret("GHL_STRIPE_SYNC_WEBHOOK_URL")`, `defineSecret("GHL_STRIPE_FAILED_WEBHOOK_URL")` — DO NOT remove Paddle secrets yet (Phase 14 deletes them)
...
- [ ] T043 [US2] Register `stripeWebhook` `onRequest` in `functions/src/index.ts` with `cors: true`, `secrets: [stripeSecretKey, stripeWebhookSecret, ghlStripeSyncUrl, ghlStripeFailedUrl]`; route to `handleStripeWebhook` (FR-014)
...
- [ ] T052 [P] [US12] Create `functions/src/billing/__tests__/ghlBillingSync.test.ts` with 5 scenarios: `notifyGHL` with uid (reads user doc), `notifyGHL` with raw email (pre-signup), `notifyGHL` POST failure logs `ghl_sync_failed` and doesn't throw, `notifyGHLFailed` generates transient `portalUrl`, `notifyGHLFailed` with portal generation failure logs `portal_session_generation_failed` and POSTs without `portalUrl`

### Implementation for User Story 12

- [ ] T053 [US12] Rewrite `functions/src/billing/ghlBillingSync.ts` `notifyGHL(identifier, event)` per contracts/stripe-webhooks.md: accept uid or raw email; resolve email/contactName from `users/{uid}` if uid; POST to `GHL_STRIPE_SYNC_WEBHOOK_URL` with `{ email, contactName, plan, billingStatus, event, credits, stripeSubscriptionId }` — `portalUrl` OMITTED (R-008, FR-018)
- [ ] T054 [US12] Add `notifyGHLFailed(identifier, event, extras?)` to `functions/src/billing/ghlBillingSync.ts`: resolve email/contactName/stripeCustomerId; if customer ID present, generate transient portal URL via `stripe.billingPortal.sessions.create({ customer, return_url })` — on failure log `portal_session_generation_failed` and continue without portalUrl; POST to `GHL_STRIPE_FAILED_WEBHOOK_URL` with `{ email, contactName, event, portalUrl?, amount?, reason? }` (R-008, FR-018)
- [ ] T055 [US12] Add fire-and-forget semantics to both helpers in `functions/src/billing/ghlBillingSync.ts`: wrap POST in try/catch; on failure log `ghl_sync_failed` and return without throwing
- [ ] T056 [US12] Wire `notifyGHL` invocations into all success-path handlers in `functions/src/billing/stripeWebhook.ts` (after billing state write): `subscription.created` (in-app + GHL paths), `subscription.updated`, `subscription.deleted`, top-up branch
- [ ] T057 [US12] Wire `notifyGHLFailed` invocations into `invoice.payment_failed` handler (event='past_due') and `charge.refunded` handler (event='refund_processed' with `amount` and `reason` extras) in `functions/src/billing/stripeWebhook.ts`
