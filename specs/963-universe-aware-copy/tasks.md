...
- [x] T012 [US1] **AS-BUILT (accepted 2026-06-25): injected at `buildFinalImagePrompt()`, not `generateConcepts`.** The original task said verify-then-inject at the scene-authoring site (`generateConcepts` ~L3100, mirroring Phase 28). The implementation instead injected `buildBlueprintMetaphorVisualBlock(resolvedUniverse)` into the single shared image-prompt assembly point `buildFinalImagePrompt()`, immediately AFTER the `BLUEPRINT:` line — the Phase 19 (gaze) pattern — because that site reaches EVERY render path (single / carousel slide / batch / retargeting / before-after / edit / reflow-rerender), which `generateConcepts` does not. Gated to `applied:true` only; subordinate to the existing identity / costume / composition rules. The Claude audit accepted this as the better universal-coverage choice (FR-005 satisfied — the instruction reaches the rendered prompt on every path); coherence is verified in localhost QA. Tests D6/D7 assert the single injection site is inside `buildFinalImagePrompt` after the `BLUEPRINT:` line. **Note**: `generateConcepts`/`generateBuildPlan` were NOT edited.
