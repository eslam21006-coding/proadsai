...
- [ ] T003 Add the additive optional `universeAwareCopy?` field to the `ResolutionTrace` interface in `functions/src/types.ts`, immediately after the `gazeDirection?` field (~L443). Shape EXACTLY per `data-model.md` § 1: `{ readonly applied: boolean; readonly styleFamily: "fantasy" | "realistic" | "minimal"; readonly reason: UniverseCopyReason }`. Include the explanatory comment block (prompt-level, never null styleFamily, additive/no-migration). Import the `UniverseCopyReason` union FROM `universeCopyMap.ts` into `types.ts` (one-directional). ⚠️ Circular-import guard: `universeCopyMap.ts` MUST NOT import anything from `types.ts` (it defines its own `StyleFamily`/`UniverseCopyReason`); if a cycle is unavoidable, instead duplicate the 6-value union as an inline literal type in `types.ts` rather than importing. Confirm `npm run build` has no circular-dependency warning.
...
- [ ] T007 In `functions/src/universeCopyMap.ts`, implement `buildFantasyMetaphorCopyBlock(resolvedUniverse: string, customUniverseDetails?: string): string` — the RELAXED rule. It MUST: permit ONE subtle evocative universe-echoing word/short phrase; forbid full thematic sentences / image-dependent copy ("must stand on its own"); state the metaphor may appear in headline, subheadline, CTA, or benefit (Gemini's choice); prefer `customUniverseDetails` text when present, else `resolvedUniverse`; and restate the Arabic-quality guardrails (no leading و, self-contained phrasing, cultural compliance — NFR-005). Advisory tone only (no "reject"/"fail" language — no enforcement pass). NOTE (FR-006): this block uses only the `resolvedUniverse` string + `customUniverseDetails` text; pulling the universe's raw `visualMotifs`/`aspirationSignals`/`tone` arrays into the prompt is DEFERRED (research.md) — add only if localhost QA shows the metaphor is too generic.
...
- [ ] T012 [US1] **VERIFY-THEN-INJECT — do NOT assume `generateBuildPlan` is the right site.** First confirm WHERE the rendered scene that feeds the image `TECHNICAL_PROMPT` is actually authored. Phase 28 injected its `EXPRESSION DIRECTION` into `generateConcepts()` (the `[VISUAL ARCHITECT V5.0]` concept prompt, ~L3100) — NOT `generateBuildPlan()` — precisely because that is where `MOOD_EMOTION` / `SUBJECT_ACTION` → `TECHNICAL_PROMPT` are authored. Trace the same path for this feature: open `generateConcepts()` (~L2928, universe block ~L2973–3083) and `generateBuildPlan()` (~L4370, universe context ~L4442–4467) and determine which one's output reaches the rendered image. Inject `buildBlueprintMetaphorVisualBlock(resolvedUniverse)` at the scene-authoring site (most likely `generateConcepts` per the Phase 28 precedent; inject into BOTH if both contribute to the rendered scene), ONLY when the decision is `applied:true` (fantasy + not suppressed). Place it subordinate to the existing `UNIVERSE LOGIC & COSTUME RULES` / identity rules so it cannot override them (FR-014: do not alter `buildFinalImagePrompt` structure). Record the confirmed site in the scratch note. **Acceptance: a fantasy generation's final rendered-image prompt (TECHNICAL_PROMPT) demonstrably contains the metaphor visual instruction** — not merely the build plan.
- [ ] T013 [US1] In `functions/src/generators.ts` `generateFinalAd()`, add the `universeAwareCopy` trace write next to the `expressionAdaptation` (~L5605) / `gazeDirection` (~L5658) writes, spreading the `UniverseCopyDecision` object computed via `resolveUniverseCopyDecision(...)` with the SAME canonical `referenceAdPresent` (T020), `isTextOnly`, and `styleFamily` signals as the copy/scene sites, plus the per-slide `isCarouselNonHookSlide` (T023). Follow the identical `_lastResolutionTrace = { ...(_lastResolutionTrace||{}), universeAwareCopy: decision }` pattern. Note: `generateFinalAd` runs once per slide for carousels (`carouselSlideIndex` is injected per call — confirmed `generators.ts` L5682–5689), so this single-object write naturally carries the current slide's decision.
...
- [ ] T020 [US3] **CANONICAL REFERENCE-AD SIGNAL (single source of truth).** Use the SIMPLEST existing presence check that is in scope at ALL THREE call sites (`generateTOV` copy site, the scene-authoring site from T012, and `generateFinalAd` trace site): `referenceAdPresent = !!(inputs as any).referenceAd` (the same check already used at `generators.ts` L323/L958). Do NOT use `referenceAdOverrideActive` (~L5678) — it is only in scope inside `generateFinalAd` and would diverge from the copy/scene sites. Wire this identical expression into every `resolveUniverseCopyDecision(...)` call (T010, T012, T013) so copy, blueprint, and trace cannot disagree on the same generation. Record the chosen expression in the scratch note so all three sites stay in sync.
...
- [ ] T023 [US4] Wire `isCarouselNonHookSlide = (inputs.adMode === 'carousel' && (carouselSlideIndex ?? 0) > 0)` into the scene-authoring/blueprint decision (the T012 confirmed site) and the trace-write decision (T013 site), so slides 2+ suppress the visual instruction AND record `carousel-non-hook-slide`. Hook slide (index 0) keeps the family-driven reason. Non-carousel (single/batch) passes `false` (FR-010 — batch needs no special logic). Use the same canonical `referenceAdPresent`/`isTextOnly`/`styleFamily` signals (T020) at this site so the decision matches the copy and trace sites.
