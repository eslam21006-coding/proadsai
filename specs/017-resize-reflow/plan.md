...
**Testing**: Plain Node.js contract fixture suite (no Vitest/Jest — repo convention is `tsc` compile + `node lib/*.test.js`). The Phase 17 fixtures land in `functions/src/contractFixtures.test.ts` (compiled to `lib/contractFixtures.test.js`) per FR-022 / 17.10 of the original spec brief, plus a parametric router-matrix test covering all 30 non-identity ratio pairs. Run via `cd functions && npm test` — that script chains the contract fixtures alongside the other suite files.
