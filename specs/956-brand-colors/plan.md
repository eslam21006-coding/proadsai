...
This plan closes those gaps without a schema migration. A new shared resolver (`brandColorResolver.ts`) computes the active brand-color pair using the precedence pinned in the spec (form input > audience avatar > inherited cold-ad > workspace) and is called from every prompt-build site in `generators.ts` (single, carousel slide loop, batch loop, retargeting flow, magic edit flow, remix flow). The existing single-image brand block is refactored to call the resolver so the same hex values flow through every site. The text compositor is extended with two optional parameters (`brandPrimary`, `brandSecondary`); when set, CTA backgrounds use the brand primary and headline accents use the brand secondary, with WCAG-luminance auto-contrast picking white-or-near-black for CTA text. A new `brandColorCompliance.ts` module performs per-asset color-extraction (Sharp `dominant` + small palette quantization) and emits per-asset compliance results onto `ResolutionTrace.brandColorCompliance[]`; `creativeScoringEngine.ts` reads those results and deducts a fixed 10 points from any flagged asset's score. The frontend gains a brand-color preview swatch component next to the existing pickers in `InputForm.tsx:1654-1690` plus a "Using workspace colors" label when the form auto-fills from the active workspace. New fixtures land in `contractFixtures.test.ts` covering all five surfaces.
