...
**Independent Test**: Switch the ad language to Arabic in Step 1 and scroll through every environment category. Confirm wine cellar, wine tasting, rooftop bar, cigar lounge, vineyard, dance studio, and the sushi-counter entry (no longer labelled "bar") are all absent or re-labelled. Switch to English and confirm all environments reappear.
...
- An Arabic user uploads a reference photo that shows a cocktail in the user's hand. The render must omit the drinking vessel or replace it with a non-alcoholic equivalent, not echo the reference literally.
...
- **FR-004**: The system MUST sanitize the visual-motif data attached to every environment so that no motif contains any of the following haram terms: cocktails, champagne, whiskey, wine, beer, spirits, cocktail reception, private bar, premium bar, bottles, barrels.
...
- **FR-006**: Motif sanitization MUST be applied at the data layer so every downstream consumer — build plan, image prompt, per-slide prompt, per-batch-item prompt — receives already-clean motifs.
...
- **FR-010**: When a saved project that references a now-blocked environment is re-opened under an Arabic configuration, the system MUST load the project fully read/editable (preserving hook, concept, copy, build-plan history, and mockup history), display an inline "Pick an Arabic-safe environment" prompt at the environment picker, and block the Generate action until the user selects an Arabic-safe environment. The project MUST NOT be force-migrated to a new environment automatically, and MUST NOT be blocked from loading or from editing non-environment fields.
...
- **FR-022**: For Arabic ads, after text-producing pipeline stages complete, the system MUST scan BOTH (a) the image-pipeline prompt (the technical-prompt text sent to the image model) AND (b) the user-facing ad copy (hook text, subhead text, caption text) for a defined list of trigger words including at minimum: wine, whiskey, cocktail, champagne, beer, alcohol, bar counter, nightclub, casino, gambling, bikini, swimsuit, lingerie, revealing, cleavage, short skirt, tank top, strapless. The same trigger list applies to both the image prompt and the ad copy.
...
- **SC-002**: 0% of Arabic ad configurations show a blocked environment (wine cellar, wine tasting, rooftop bar, cigar lounge, vineyard, dance studio) in the environment picker, verified by opening the picker in Arabic and confirming the absence of every blocked entry.
