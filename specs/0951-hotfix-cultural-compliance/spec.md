...
- **FR-007**: When the ad language is set to Arabic, the system MUST hide from every environment picker any environment whose Arabic-safety flag is not true. Hidden environments MUST be absent from the UI, not greyed-out or marked as locked. **Fail-closed rule:** any referenced universe identifier — whether in a live form, a draft, an audience avatar, or a loaded saved project — that does NOT resolve to a current universe entry MUST be treated exactly the same as an `arabicSafe: false` entry (unresolved ≡ blocked). A universe that was renamed MAY be auto-resolved via the legacy-id remap (see FR-011); an identifier that cannot be resolved by that remap MUST fail-closed rather than be treated as "safe by default".
