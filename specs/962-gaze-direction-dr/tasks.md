...
- [ ] T005 [US1] Add Contract A + B + E test cases to `functions/src/__tests__/gazeMap.test.ts`: resolver coverage A1–A11 (10 hooks, 12 objections, aliases, unknown→fallback no-throw, null→null, hook>objection priority), image-block B1–B6 (GAZE DIRECTION text, identity clause, empty-space/cross-eyed/robotic prohibitions, 9:16 vertical note, before/after split, null→""), and trace E1–E4 shape assertions
...
- [ ] T010 [US1] In `functions/src/generators.ts` `buildFinalImagePrompt()` (~line 5428, immediately AFTER the Phase 28 expression block, after BLUEPRINT), inject the result of `buildImagePromptGazeBlock(resolveGazeDirective({coldHookAngle, retargetingObjection|retargetingObjections[0]}), { beforeAfterSplit: isBeforeAfterSelection(inputs, inputs.coldHookAngle), aspectRatio })`; comment out nothing it replaces is new (additive) — guard so a null directive injects nothing
