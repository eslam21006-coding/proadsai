...
- [ ] T027 [US3] Handle deletion of the workspace the member is currently using in `src/App.tsx` — reuse the existing switch-guard dialog (`WorkspaceSwitcher.tsx:174-202`, driven by `hasInProgressWork`) rather than building a second mechanism: when the active workspace disappears from the live snapshot and `hasInProgressWork` is true, open the guard with the account's default workspace as the target so the member chooses save or discard. **Do not advance `activeWorkspaceId` while the guard is open** — the auto-save effect derives the project's `workspaceId` from it and lists it as a dependency, so switching first re-queues the in-flight work tagged with the destination workspace and saves it under a workspace the member never worked in. If the member chooses Save & Switch, `await autoSaveForceFlush()` **before** the switch proceeds and surface a plain-language error if the flush fails; only then advance the workspace. When `hasInProgressWork` is false there is nothing to attribute, so move to the default immediately. In every case show a plain-language notice that the workspace is no longer available (AS-3.3, FR-017)
