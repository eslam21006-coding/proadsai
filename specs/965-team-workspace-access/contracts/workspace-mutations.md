...
`deleteWorkspace` (:6431), `restoreWorkspace` (:6474), `linkMetaAccountToWorkspace` (:6545),
`unlinkMetaAccountFromWorkspace` (:6591); new `assertNotTeamMember()` in
...
|---|---|---|---|---|
| M1 | Owner | create / update / delete / restore / linkMeta / unlinkMeta | **allow** — behaviour byte-for-byte as today | — |
...
| M5b | `isTeamMember=true` (any role) | `linkMetaAccountToWorkspace` | **deny** | `permission-denied`, `reason: team_member` |
| M5c | `isTeamMember=true` (any role) | `unlinkMetaAccountFromWorkspace` | **deny** | `permission-denied`, `reason: team_member` |
