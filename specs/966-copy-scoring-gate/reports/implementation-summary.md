...
5. **Best-of.** A rewrite is accepted only if it scores strictly better than the original AND passes the threshold. If a rewrite scores lower, it is discarded with `rejectReason: "scored_lower"` or `"below_threshold"`. Two rewrite passes maximum — no loops. Best-of-candidate selection ensures the original block ships if no rewrite helps.
