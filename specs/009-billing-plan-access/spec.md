...
2. **Given** a user who confirms cancellation, **When** processing completes, **Then** the Billing page header shows "Cancelled — access until [date]" and the subscription is set to cancel at period end (not immediately).
3. **Given** a cancelled user before their period end date, **When** they use the app, **Then** all features remain accessible until the cancellation date.
...
- **FR-010**: System MUST implement cancellation flow: user initiates cancellation with confirmation dialog showing period end date, subscription is set to cancel at period end (not immediately), and the UI reflects the pending cancellation with the access end date.
