...
- **FR-076**: The per-sync summary line MUST report **how many contributions came from unlinked rows** — rows that resolved to no `generationId`. This count MUST be emitted on every sync, including when it is zero.
  - *Justification*: this number is the health signal for perceptual matching. A rising count means matching is degrading, and without it that degradation is silent. It is testable: seed a sync with a known number of unlinked rows and assert the emitted count equals it. The count is added to the list in FR-051b.
  - *Current baseline*: **5 of 1008 rows (0.5%) carry a `generationId` at all.** Account `act_995888422231015` has **zero** linked rows, so on that account every row resolves through the `imageHash` path and the `generationId` branch of FR-074 is currently **inert**. This is the starting condition the count must make visible.
...
- **FR-036a**: PROVISIONAL contributions MUST count toward the activation threshold and toward the per-item floor in FR-034a. A creative is PROVISIONAL for this purpose when it carries no sealed target; the state is a property of the creative, not of an individual row.
...
- **The learning eligibility rules are unchanged.** Only ads with a confirmed creative link, available metadata, and a resolvable angle or pattern contribute. This feature changes how contributions are recorded and *counted*, not which ads qualify. Amendment 1 does not relax `isEligibleForLearning` (`functions/src/learningAggregates.ts:119-124`); it changes the unit the surviving contributions are counted in.
