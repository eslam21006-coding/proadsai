...
- [ ] T014 Replace the unbounded `adPerformance` collection scan at `functions/src/metaSync/shared.ts:831` with a chunked by-ID read over exactly the current batch, chunk size **300**, every chunk completing before any write (FR-067, FR-069). **Remove the old read entirely** (FR-068)
...
- [ ] T056 [US4] Add the plain-language multi-funnel indication to `functions/src/whatsWorkingDashboard.ts` and the two bilingual strings to `src/i18n.tsx` **only if the indication requires new copy** (FR-041, FR-047). Arabic must be simple Fusha, no dialect, no jargon, no raw measurement values (FR-048, FR-049)
- [ ] T057 [P] [US4] Write dashboard tests in `functions/src/__tests__/whatsWorkingAllTime.test.ts` covering SC-007, SC-009, SC-010, and SC-021 (FR-065's existing bilingual message carries **zero** technical terms and **zero** raw measurement values — the string is unchanged by this feature, but the assertion is not currently made anywhere); **and register the pre-existing `whatsWorkingDashboard.test.js`, which is present in the test directory but absent from the runner manifest** (FR-050, SC-011)
