...
- [ ] T015 Make a failed chunk read abort the learning write for that chunk rather than read as "no prior contribution" in `functions/src/metaSync/shared.ts` (FR-070) — conflating them produces the double-count FR-018 forbids. **Status after Batch 04 (corrected): complete-as-specified.** Phase 2 establishes the `failedLedgerReads` set; Phase 3's T018b consumes it field-level via the `decideAdWrite` helper in `functions/src/learning/fieldLevelDiscrimination.ts`. The behavioural test FR-070 names by assertion (`fr070.test.ts`) drives `decideAdWrite` with synthetic inputs and asserts on returned values — both the "no contribution was added" half and the operational-fields-still-written half.
- [ ] T016 [P] Write bounded-read tests in `functions/src/__tests__/boundedLedgerRead.test.ts` covering SC-022 (chunk-failure surfaces in `failedIds`), SC-023 (chunking at 300, batch-size bounded), SC-024 (no `collection("adPerformance").get()` call remains), SC-025 (whole-document return, cascade fields preserved). **Coverage split — corrected in Batch 02b**: the 12 tests cover FR-070's **producing half** (that `readExistingAdDocs` reports failed ids correctly) and FR-071 (whole documents). They do **not** cover FR-070's **consuming half** — the test FR-070 requires by name: "a test that forces a read failure and asserts no contribution was added". The consuming half lands in T018b below. Register the file in `functions/package.json`
...
- [ ] T017 [P] [US1] Implement `decideContribution` in `functions/src/learning/contributionLedger.ts` per `contracts/contributionLedger.md` — the add / no-op / withdraw-then-add / withdraw-only decision table (FR-016, FR-017). **Status after Batch 04: complete-as-specified.** `decideContribution` is implemented and tested in `learningAccumulation.test.ts`'s `decideContribution` block (5 assertions covering all four decision-table rows + the both-null case).
- [ ] T018 [US1] Implement per-record delta application in `functions/src/learning/aggregateDelta.ts` — atomic increments for sums, counts, win/loss totals and the per-funnel-type breakdown; a withdrawal is a **negative delta**, never a recomputation (FR-021, research.md §D3). **Status after Batch 04: complete-as-specified.** `applyHookAggregatesDelta`, `applyVisualAggregatesDelta`, and `applyHookAggregateWithdrawal` are implemented. `shared.ts:1161-1162` uses the additive path; partial syncs preserve historical evidence. Test surface in `learningAccumulation.test.ts` (SC-002/008/013/029c).
- [ ] T018a [US1] **(moved from Phase 2; rationale recorded there)** Add the pre-commit fencing re-check and abort path in `functions/src/metaSync/shared.ts` — re-verify holding immediately before committing, abort the learning write without failing the sync, record the event (FR-062, FR-064, FR-052). **Status after Batch 03: complete-for-the-phase-surface** (code in place at `shared.ts:1382-1415`; behavioural test of lease-loss is T064b in Phase 7).
- [ ] T018b [US1] **(rewritten batch-03 per owner, completed batch-04 with behavioural test)** In `functions/src/metaSync/shared.ts`, consume the `failedLedgerReads` set in the per-ad loop with **field-level discrimination**, not write-level suppression. **Status: complete-as-specified** at the code level (`decideAdWrite` in `learning/fieldLevelDiscrimination.ts`) AND at the test level (`fr070.test.ts` is now BEHAVIOURAL — drives the helper with synthetic inputs and asserts on returned values, not on source text). Owner correction to Batch 03 (this batch's §1) replaced the prior source-text assertions with seven behavioural assertions covering all three required halves:

  - **Skip the precedence lock** at `shared.ts:900-905` (do not re-derive `matchType`/`generationId` from fresh Meta data — a failed read is missing information, not absence).
  - **Skip pushing the ad into `learnedAds`** — failed-read ads contribute nothing to the learning aggregates this sync.
  - **Do NOT skip the `writes.push(...)` call.** Build an `adDoc` that omits the **linking fields** (`generationId`, `matchType`, `matchDistance`, `metadataAvailable`, `linkProvenance`, `deletedGenerationId`) and contains only the **operational fields** (spend, conversions, click-through, verdict, geo tier, audience type, creative type, image hash, age, etc.). With `merge: true` on the write (the existing semantics at `shared.ts:1222-1226`), this:
    - **Re-sync**: preserves the existing linking fields unchanged (merge keeps fields not present in the new data) and updates operational fields to today's Meta values.
    - **First-ever sync**: creates a doc with operational fields only; the ad has no linking and contributes nothing (correct — we have no prior evidence).
  - **Surface the failed IDs in `errors[]`** for observability (T062 reads from this surface).
- [ ] T018c [P] [US1] **(added batch-02b; SC-049 interim tripwire)** Write a structural regression check that asserts the **last** `batch.commit()` call inside `runSyncForAccount` (in `functions/src/metaSync/shared.ts`) precedes the **first** `acquireLearningLease` call (T013 lease primitive). Implemented by reading `shared.ts` as text and finding the line numbers of these two call shapes, asserting `max(batch.commit() line) < min(acquireLearningLease line)`. The test's header comment must state plainly that this is a **necessary but not sufficient** condition for SC-049, that it is a tripwire against reordering and not coverage of the criterion, and that it is **retired when T064b (Phase 7) lands** with a `describe.skip` annotation. Register the file in `functions/package.json` (T067 reach-the-end). The reason this is needed: Phase 3 lands the learning write between these two lines and is the phase most likely to reorder them; the four phases that follow (3–6) would otherwise edit the most-reorganised code without verification, with SC-049's behavioural test arriving only in Phase 7.
- [ ] T019 [US1] Invert the contract of `functions/src/learningAggregates.ts` from overwrite to delta, and **delete the module-header rule at line 17** (*"Same generationId in 2 ad sets → separate records per context"*), which FR-073 overturns. **Status after Batch 05: complete-as-specified AT THE GENERATION LEVEL.** The line is deleted and replaced with the FR-073 statement, AND `shared.ts` calls the additive aggregator (T018). **Scope: same as T018 — per generation, not per creative.** The headline behaviour FR-073 names ("creative contributes exactly once") is structurally enabled by the additive aggregator but not yet enforced end-to-end. Batch 05 also drops the Batch 04 claim that the legacy `updateHookAggregates` / `updateVisualAggregates` remain for `ragContext.ts` — a module-wide grep shows no such caller. The legacy functions are dead code outside tests. **Proposal deferred to follow-up batch per owner instruction**: delete `updateHookAggregates`, `updateVisualAggregates`, and the `learningAggregates.test.ts` + `learningIntegration.test.ts` files (both test the OVERWRITE contract the feature has replaced). Until then, `shared.ts`'s dead imports are removed in this batch to silence the dead-import warning.
...
- [ ] T021 [US1] Make counts count **distinct creatives**, not ad rows, in `functions/src/learningAggregates.ts` and `functions/src/learning/aggregateDelta.ts` (FR-036, FR-073). **Status after Batch 06: complete-for-the-phase-surface** (corrected from Batch 05's "complete-as-specified"). The aggregator's grouping machinery is in place (`groupAdsByCreative` groups by `creativeKey` with `adId` fallback; any-row eligibility; all-rows aggregation). **But** the worker in `shared.ts` sets `creativeKey: ad.id` for every row, so grouping behaviour is currently identical to per-row — every group has one member. The discriminating SC-008 test (one creative contributes 1, not 55) fails while `creativeKey` is `ad.id`. Real per-creative aggregation lands with T021a (worker integration with `groupIntoCreatives`).
- [ ] T021a [US1] **(added batch-06; addresses Batch 05 finding 1)** Wire `groupIntoCreatives` into the worker's learnedAds construction in `functions/src/metaSync/shared.ts`. The per-ad loop's `creativeKey` field on `AdForLearning` must come from the `CreativeGroup.creativeKey` returned by `groupIntoCreatives(rows)`, NOT from `ad.id` (Batch 05's placeholder). With this wire-up, SC-008's per-creative form is asserted: a 55-row creative contributes ONE count in the angle, not 55. **Discriminating assertion**: the new test (added to `learningAccumulation.test.ts`) drives `groupIntoCreatives` + `applyHookAggregatesDelta` together and asserts `count === 1` for 55 rows in one creative — that assertion **fails** while `creativeKey` is `ad.id` (Batch 05's placeholder) and **passes** when the real key flows. Lands in the follow-up Phase 3 close-out batch.
- [ ] T022 [US1] Implement any-row eligibility and all-rows aggregation in `functions/src/learning/aggregateDelta.ts` (FR-074g) — a creative is eligible if **any** row qualifies, and **every** row's values then aggregate. **Status after Batch 06: complete-for-the-phase-surface** (corrected from Batch 05's "complete-as-specified"). The eligibility and aggregation logic is in the aggregator (`groupAdsByCreative` → filter `isAdEligible` → all-rows aggregation per group), but with `creativeKey: ad.id` for every row the groupings are all single-member, so any-row eligibility is unexercised in practice. Per-creative behaviour lands with T021a.
...
- [x] T025a [US1] **(added batch-12; addresses Batch 11 §5 re-verification)** DONE. The post-pass generation patch in `functions/src/metaSync/shared.ts` now flows the resolved `entry.hookAngle` (from `gen.input.coldHookAngle`/`hookAngle`) and `entry.layoutTemplate`/`creativeModes`/`artDirection`/`universe` (from the resolved generation doc) back into the queued `adDoc.ledger.angleKey`/`ledger.patternKey` (via `computePatternKey`). Approach: complete-there — the post-pass loop mutates the queued write's ledger entry rather than moving `writes.push` after the patch (which would have restructured the chunked-commit plumbing at lines ~1300). The wiring is inside the existing post-pass loop, so the same `if (!gen) continue` semantics apply: ads with a genMap miss keep null keys (preserving the "no resolved keys known" signal), failed-read ads never reach `learnedAds` so their `adDoc.ledger` stays undefined (FR-070: no contribution → no ledger entry). **Discriminating assertion** (added to `t025aWorkerWiringDiscriminator.test.ts`): three tests observe the WORKER'S output (the queued `writes[i].data.ledger`), not the function's in isolation — (1) BEFORE wiring, the simulated worker queues null ledger keys; (2) AFTER wiring, the simulated worker queues the resolved `angleKey="urgency"` and the `computePatternKey(layoutTemplate, modes, artDirection, universe)` value; (3) SOURCE-TEXT structural check verifies `shared.ts` contains the wire-up lines (the line check rejects commented-out occurrences). Both halves demonstrated: reverting the wiring in `shared.ts` causes the SOURCE-TEXT check to fail (exit 1); restoring it returns the suite to 3/3 (exit 0). FR-013/017's withdraw-then-add path and FR-051a's audit guarantee both now have the keys they need to find and explain prior contributions.
...
- [ ] T027 [P] [US1] Write delete-cascade tests in `functions/src/__tests__/phase969/learningCascade.test.ts` — metadata loss prevents further contribution but **never withdraws** one already made (FR-014), and a creative is never half-cascaded because the cascade queries by `generationId` (FR-074g); register the file in `functions/package.json`. **Status after Batch 06: complete-as-specified.** Four assertions in `learningCascade.test.ts`: (1) cascade-marked creative no longer contributes going forward (behavioural); (2) previous contribution STANDS through cascade — no withdrawal (behavioural); (3) **SOURCE-TEXT** check that `applyAdToHook` has no decrement patterns (relabelled per Batch 06 finding 3 — necessary-but-not-sufficient structural guard, with the new (4) below as the behavioural counterpart); (4) **NEW** behavioural: a creative never half-cascades (every row of a cascade-marked creative has metadataAvailable=false and is filtered).
- [ ] T028 [US1] **(added batch-07; addresses Batch 07 finding 4 — extracting per-ad learning into a pure function)** Extract the per-ad learning section of `runSyncForAccount` in `functions/src/metaSync/shared.ts` into a pure function `decideAdWriteActions(input, varying)` in `functions/src/learning/decideAdWriteActions.ts`. The function returns the full per-ad action set (whether to contribute, the adDoc shape with optional ledger, the matched/ambiguous/unmatched tally, the resolved generationId, and the `AdForLearning` entry). **Status after Batch 07: the pure function and its tests exist; the `shared.ts` refactor (replacing the inline per-ad block with a call to the function) is deferred to Batch 08.** Tests in `perAdActions.test.ts` (11 assertions) drive the function directly with synthetic inputs — T021a discriminator at the WORKER level (fails under Batch 05's `creativeKey: ad.id` placeholder, passes when T021a's wire-up lands), T025a ledger keys populated when `resolvedHookAngle` is set, FR-070 wiring behavioural (failed-read ads do not contribute; operational fields preserved). `fr070Wiring.test.ts` was deleted — superseded by the behavioural checks in `perAdActions.test.ts`.
...
- [ ] T029a [US1] **(added batch-09; addresses Batch 09 §3 — gate migration producer side, ragContext)** Populate `creativeCount` (the distinct-creative count field added by T021) on every hook aggregate emitted by `functions/src/ragContext.ts`. **Gating dependency**: the FR-034 / FR-034a / FR-037 gates in `getTopWinners.ts` (T050) cannot read `creativeCount` until it is non-undefined on the Summary aggregate they consume. Today `creativeCount` is read with `?? sampleSize`, which silently falls back to the **per-row** count for every gate. The owner-locked decision is **10/3/3 counted in distinct creatives**; that decision is **not enforced in code** until T029a/b/c land and the `?? sampleSize` fallback is removed in T050. **Discriminating assertion** (added to `ragContext.test.ts`): driving `summarizeAccount` with N creatives × M rows each produces a `Summary` whose hook aggregates have `creativeCount === N` (not `N*M`). Behavioural. **Depends on**: T021 complete-as-specified AT THE GENERATION LEVEL (Batch 05), T021a's wire-up discriminator (Batch 09 — 3 assertions in `t021aWireupDiscriminator.test.ts`). **Status after Batch 09: NOT STARTED**.
- [ ] T029b [US1] **(added batch-09; addresses Batch 09 §3 — gate migration producer side, whatsWorkingDashboard)** Populate `creativeCount` on every hook aggregate emitted by `functions/src/whatsWorkingDashboard.ts`. **Same gating dependency as T029a**: the dashboard reads the same `Summary.hookAggregates[]` shape and inherits the same `?? sampleSize` fallback. Until T029b lands, the dashboard's per-angle `creativeCount` is the per-row `sampleSize`, which makes the dashboard's headline figure (FR-040 all-time totals, FR-039 tier icons) silently wrong. **Discriminating assertion** (added to `whatsWorkingAllTime.test.ts` from T057): driving the dashboard reader with the same fixture as T029a returns `creativeCount === N` per angle. Behavioural. **Depends on**: T053 (the dashboard read-side update), T021/T021a. **Status after Batch 09: NOT STARTED**.
- [ ] T029c [US1] **(added batch-09; addresses Batch 09 §3 — gate migration producer side, rankingEngine)** Populate `creativeCount` on every hook aggregate emitted by `functions/src/rankingEngine.ts` (the Summary producer). **This is the root producer** — `ragContext.ts` (T029a) and `whatsWorkingDashboard.ts` (T029b) both consume this `Summary`. Until T029c lands, T029a/T029b have nothing to populate. **Discriminating assertion** (added to `rankingEngine.test.ts` or the equivalent producer-side test): the `Summary` builder returns hook aggregates with `creativeCount === N` (distinct creatives), distinct from `sampleSize` (per-row). Behavioural. **Depends on**: T021/T021a. **Status after Batch 09: NOT STARTED**.
...
- [ ] T049 [US3] Implement funnel-type **weighting** in the retrieval path in `functions/src/getTopWinners.ts` — weight toward same-funnel evidence, **never filter, exclude, suppress or zero** (FR-030, FR-031, FR-032a)
  - **Batch 18 note (recorded 2026-09-06, FR-030 implementer MUST read):** `byFunnelType.{paid_event,paid_product,free_webinar,lead_magnet_call,unknown}.count` stores **row counts**, not creative counts. The Batch 17 dashboard `multiFunnel` boolean reads `count > 0` per bucket (correct). Any **weighting** that reads `byFunnelType` directly will recreate Batch 06's 7.4:1 fan-out inflation per funnel — one creative with 55 rows under one funnel would outweigh a creative in another funnel 55:1 in any popularity-weighted score. Mirror the FR-073 / Batch 06 fix the existing `byObjective` path already uses: a `byFunnelTypeCreativeCount` parallel field counted in **distinct creatives per bucket** (a per-bucket `Set<creativeKey>` or an equivalent), populated in `applyAdToHook`/`applyAdToVisual`/the withdrawal alongside `count`. The seam: "row counts are sufficient for the FR-041 boolean (Batch 17); FR-030's weighting MUST NOT consume `byFunnelType.count` directly." See the long comment on `ByFunnelTypeBreakdown` in `functions/src/learningAggregates.ts`.
...
- [ ] T056 [US4] Add the plain-language multi-funnel indication to `functions/src/whatsWorkingDashboard.ts` and its two bilingual strings to `src/i18n.tsx` (FR-041, FR-047). **This is the one new owner-visible string this feature introduces** — it has nothing to display before this feature, because cross-funnel evidence does not exist until it ships. Arabic must be **simple Fusha**: no dialect, no jargon, no raw measurement values, no percentages (FR-048, FR-049).
  - **OWNER REVIEW REQUIRED BEFORE THIS SHIPS.** Surface the proposed English and Arabic to the owner and **wait** for approval. Do **not** treat SC-010 passing as the review: SC-010 confirms the string exists in both languages and carries no jargon; it **cannot** judge whether the Fusha reads naturally to a Gulf coach, which is what Constitution Principle V requires. This is the only new owner-visible string in the feature, and the plan's Constitution V verdict is **"PASS, with work"** on the strength of this review happening.
...
- [ ] T064b [P] **SC-049 behavioural test (added Batch 02a).** Write an end-to-end test that drives `runSyncForAccount` with a stubbed Firestore and a stubbed Meta fetch path, pre-populates the `learningLeases/{ownerUid}_{accountId}` doc with a different `runId` so the lease acquire is refused, asserts the operational status writes for every ad in the batch were committed (the Firestore stub records them), and asserts `SyncResult.status === "failed"` is returned. Both halves are required in one test (SC-049): zero ads left with a stale operational status AND the failure signal emitted. Register in `functions/package.json`. The Phase 2 grep-based claim ("structurally satisfied") is insufficient — a refactor that moves the commit inside a conditional satisfies the grep and breaks the criterion. This test is end-to-end and naturally lives in Phase 7 alongside the observability tests, not Phase 2's unit tests. Source-level claim from Batch 02 §5.4 is **withdrawn**. **Additional obligation added in Batch 12 review**: when the stubbed Firestore + stubbed Meta scaffolding exists, T064b must ALSO assert the worker's real output for the two wire-ups whose interim regression guards are currently SOURCE-TEXT (not behavioural):
  1. **T021a wire-up** — drive `runSyncForAccount` end-to-end; assert that the per-ad block's `decision.adDoc.ledger.creativeKey` (output by `decidePerAdActionsForWorker`) is the actual creative key from `groupIntoCreatives`, not `ad.id`. The current SOURCE-TEXT guard at `t021aWireupDiscriminator.test.ts:217` catches `shared.ts` losing the `resolveCreativeKeyByAdId(` call and the `creativeKeyByAdId.get(ad.id)` lookup; T064b replaces the SIMULATION-only verification at `perAdActions.test.ts:101-181` and `t021aWireupDiscriminator.test.ts:172-216` with a real observation.
  2. **T025a wire-up** — drive `runSyncForAccount` end-to-end; assert that the queued write's `data.ledger.angleKey` is the post-pass resolved `entry.hookAngle` and `data.ledger.patternKey` is `computePatternKey(entry.layoutTemplate, entry.creativeModes, entry.artDirection, entry.universe)`. The current SOURCE-TEXT guard at `t025aWorkerWiringDiscriminator.test.ts:313` catches `shared.ts` losing the post-pass ledger-key mutation; T064b replaces the SIMULATION-only verification at `t025aWorkerWiringDiscriminator.test.ts:195-289` and `perAdActions.test.ts:243-290` with a real observation.
  Both checks land in T064b's single end-to-end test file (or split if the helper signatures diverge). When T064b lands, the SOURCE-TEXT guards in `t021aWireupDiscriminator.test.ts` (line 217) and `t025aWorkerWiringDiscriminator.test.ts` (line 313) retire with `describe.skip` annotations, and the SIMULATION behavioural halves are deleted — the behavioural verification moves with the production code to Phase 7's real observation. Until T064b lands, the SOURCE-TEXT guards stand as the **interim** regression guard.
