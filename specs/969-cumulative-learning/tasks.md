...
- [ ] T025a [US1] **(added batch-09; addresses Batch 08 finding 1 — ledger keys deferred)** Ensure the `decideAdWriteActions` per-ad extraction in `functions/src/learning/learningPerAdLoop.ts` populates the **ledger key fields** (`generationId`, `matchType`, `matchDistance`, `linkProvenance`, `metadataAvailable`, `deletedGenerationId`) on the `adDoc` shape returned for the `writes.push(...)` call when `resolvedHookAngle` is set (T028 references this in its status). Today the per-ad function returns the ledger entry but the `writes.push` site in `shared.ts` does not spread the keys onto the ad document — they live only on the ledger side and the `adDoc` is operational-fields-only. **Discriminating assertion** (added to `perAdActions.test.ts`): with a `resolvedHookAngle` set, `decidePerAdActionsForWorker` returns an `adDoc` whose keys include all six ledger fields AND the operational fields, and the returned `AdForLearning` row's `creativeKey` matches the `creativeKeyByAdId` map entry. Behavioural: not source-text. **Depends on**: T028 complete (per-ad extraction landed in Batch 09 at commit 5fa927b). **Status after Batch 09: NOT STARTED** — the per-ad extraction exists but `writes.push` still spreads the same operational-only shape Batch 08 used. **Status after Batch 10**: re-verified (item 2 of Batch 10 report): the task obligation remains unrecorded-as-done. **Status after Batch 11: NOT DONE — function capable, worker not wired.** `perAdActions.test.ts` shows `decidePerAdActionsForWorker` populates `ledger.angleKey` and `ledger.patternKey` from `resolvedHookAngle` / `resolvedPatternKey` when given resolved values (4 of the 11 assertions are T025a discriminator checks), but the worker-side `writes.push(...)` call in `metaSync/shared.ts` still spreads the operational-only shape, so the live ledger entries are written with `angleKey: null` and `patternKey: null`. Same shape as T021a was before Batch 09's wire-up: the function in isolation is correct, the integration has not been wired. Deferred.
