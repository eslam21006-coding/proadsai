...
- [ ] T015 Make a failed chunk read abort the learning write for that chunk rather than read as "no prior contribution" in `functions/src/metaSync/shared.ts` (FR-070) — conflating them produces the double-count FR-018 forbids. **As implemented in Phase 2 (batch-02-969-report.md §3): partial.** Phase 2 establishes the `failedLedgerReads` set as a property of the bounded-read result. The per-ad loop's consumption of it — the actual write-abort logic for failed chunks — is T018b in Phase 3. **An implementation that captures the set without consuming it is structurally the FR-070 bug.** The set's existence is a precondition for T018b, not its completion.
...
- [ ] T018a [US1] **(moved from Phase 2; rationale recorded there)** Add the pre-commit fencing re-check and abort path in `functions/src/metaSync/shared.ts` — re-verify holding immediately before committing, abort the learning write without failing the sync, record the event (FR-062, FR-064, FR-052). **Must be inserted between the delta computation and the delta write**, with the abort path consuming the same `failedLedgerReads` set **T018b consumes** (the bounded read produced it in T014; T018b is what feeds it into the abort path). Per FR-063 the residual window between re-verify and commit is acknowledged, not papered over
- [ ] T018b [US1] **(added batch-02b; rewritten batch-03 per owner)** In `functions/src/metaSync/shared.ts`, consume the `failedLedgerReads` set in the per-ad loop with **field-level discrimination**, not write-level suppression. For an ad whose bounded-read chunk failed (FR-070):
  ...
  **Field-level, not write-level.** Suppressing the entire write would freeze verdict, spend, and operational status against FR-009 and SC-049's protection ("operational status current for every ad in the batch"). The discrimination is between **fields derived from prior contribution data** (linking fields, which we cannot safely re-derive) and **fields computed from current Meta data** (operational fields, which we can and should always write).

  **Required test, by name**: "a test that forces a read failure and asserts no contribution was added" (FR-070's own words). Add to `functions/src/__tests__/phase969/learningAccumulation.test.ts` (T026) or a dedicated `fr070.test.ts`: assert that a failed-read ad has no entry in `learnedAds` AND that the same ad's persisted adPerformance doc has its updated operational fields (e.g. `spend3d`, `conversions3d`) but **no change** to `matchType`/`generationId` relative to the prior doc — both halves in one test, since a single-half assertion could pass under either interpretation.
