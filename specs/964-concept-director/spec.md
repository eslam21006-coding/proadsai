...
- **FR-015**: When the validator flags a blocking duplication on a sibling concept that has not yet been retried, the system MUST regenerate that specific concept by re-running the Director with the duplicated markers (across ALL blocking axes for that concept — metaphor / layout / headline — not just the first one encountered) appended to that concept's avoid-list. This retry MUST be limited to **at most one per offending concept** (per-concept guard, not per-request), and other offending siblings MAY retry independently up to their own per-concept limit. The `retryCount` trace counter records how many concepts were retried (≤ `conceptCount`).
- **FR-016**: If a concept's retry still fails validation, the system MUST ship the concepts as-is (with `varianceAchieved:false` on the trace), perform no further retries, and record that full variance was not achieved. The validator MUST NEVER block or fail the user's generation — `varianceAchieved:false` is an audit signal, not a generation failure.
