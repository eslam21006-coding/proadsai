...
- [ ] T021 [US4] Implement the flag + kill-switch reads (Contract C2 / D4): per-user `users/{uid}.conceptDirectorEnabled` (absent/non-boolean → `false`), and Remote Config `conceptDirectorKillSwitch` via the Admin SDK server template with a **60s in-process cache** that serves last-known-good on read error. Place in a small helper `functions/src/conceptDirectorConfig.ts` (or inline in `index.ts` if simpler — keep `conceptDirector.ts`/`varianceValidator.ts` pure either way).
