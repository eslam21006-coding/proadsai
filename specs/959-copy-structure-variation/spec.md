...
- **Carousel contract location**: Section 17 of the reference points to "Section 5.A of COPY_SYSTEM_REFERENCE.md" for the carousel middle-slide plan, but the current reference document has no Section 5.A. The committed carousel slide-plan contract actually lives at `specs/001-resolver-completeness-trace/contracts/carousel-slide-count-plan.md` (and Section 6 of the reference). This spec assumes "Section 5.A" refers to that committed contract; the same-PR sync requirement (FR-022) targets that contract file and the relevant reference section, and the missing section reference should be reconciled during planning.
