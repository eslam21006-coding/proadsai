...
- Q: Publishing today only uploads the image to the ad account's media library — no ad, ad set, or campaign is created. The Facebook Page is treated differently by the two publish paths: the single-creative path stores it as metadata that no Meta call consumes, but the `metaPushCreativePack` path sends the resolved Page as `page_id` to `/adcreatives` via `object_story_spec.page_id`. Should publishing still be blocked when a workspace has no Page? → A: No — but only for the single-creative path where no Meta call actually consumes the Page. The per-workspace Page is still recorded, switched, and cleared exactly as specified, so the data is correct and ready for when ad creation is built. The pack path resolves and consumes the Page the same way it always has, so the existing FR-015a / FR-015b behaviour for that path is preserved. The ad-account gate stays in both paths because the upload genuinely uses it.
